...
<div class="max-w-6xl mx-auto py-10 px-4">
    <h1 class="text-3xl font-bold text-blue-800 mb-6">Data Koreksi Nilai</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-blue-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Mata Kuliah</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Dosen Pengampu</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Semester</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Alasan Koreksi</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">{{ $item->dosen_pengampu }}</td>
                        <td class="px-4 py-3">{{ $item->semester }}</td>
                        <td class="px-4 py-3">{{ $item->alasan_koreksi }}</td>
                        <td class="px-4 py-3">{{ $item->created_at->format('d-m-Y') }}</td>

                        <!-- Status -->
                        <td class="px-4 py-3">
                            <form action="{{ route('admin.koreksi-nilai.status', $item->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                                @method('PUT')
                                <select name="status"
                ...
                @empty
                    <tr>
                        <td colspan="10" class="px-4 py-5 text-center text-gray-400">Belum ada data.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
