...
<div class="max-w-6xl mx-auto py-10 px-4">
    <h1 class="text-3xl font-bold text-blue-800 mb-6">Data Surat Pengantar Kerja Praktik</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-blue-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Nama Perusahaan</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Alamat Perusahaan</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Tujuan Surat</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Lama Magang</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2">{{ $item->alamat_perusahaan }}</td>
                        <td class="px-4 py-2">{{ $item->tujuan_surat }}</td>
                        <td class="px-4 py-2">{{ $item->lama_magang }}</td>
                        <td class="px-4 py-2">{{ $item->created_at->format('d M Y') }}</td>

                        <!-- Status -->
                        <td class="px-4 py-2">
                            <form action="{{ route('admin.kerja-praktik.status', $item->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                                @method('PUT')
                                <select name="status"
                ...
                @empty
                    <tr>
                        <td colspan="11" class="px-4 py-5 text-center text-gray-400">Belum ada data.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
