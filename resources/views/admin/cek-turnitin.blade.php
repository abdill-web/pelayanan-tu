...
<div class="max-w-6xl mx-auto py-10 px-4">
    <h1 class="text-3xl font-bold text-blue-800 mb-6">Data Pengajuan Cek Turnitin</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-blue-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">File</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            @if ($item->file_pdf)
                                <a href="{{ route('admin.lihat-turnitin', ['filename' => $item->file_pdf]) }}" target="_blank" class="text-blue-600 hover:underline">
                                    Lihat File
                                </a>
                            @else
                        ...
                        <!-- Status -->
                        <td class="px-4 py-3">
                            <form action="{{ route('admin.turnitin-request.status', $item->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                                @method('PUT')
                                <select name="status"
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">
                            Terakhir:<br>
                            <span class="text-xs">{{ $item->updated_at->format('d-m-Y H:i') }}</span>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="9" class="px-4 py-5 text-center text-gray-400">Belum ada data.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
