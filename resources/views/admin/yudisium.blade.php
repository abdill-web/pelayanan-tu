...
<div class="max-w-6xl mx-auto py-10 px-4">
    <h1 class="text-3xl font-bold text-blue-800 mb-6">Data Yudisium</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-blue-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Angkatan</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">IPK</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Judul TA</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">{{ $item->ipk }}</td>
                        <td class="px-4 py-3">{{ $item->judul_ta }}</td>
                        <td class="px-4 py-3">{{ $item->created_at->format('d-m-Y') }}</td>

                        <!-- Status -->
                        <td class="px-4 py-3">
                            <form action="{{ route('admin.yudisium.status', $item->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                                @method('PUT')
                                <select name="status"
