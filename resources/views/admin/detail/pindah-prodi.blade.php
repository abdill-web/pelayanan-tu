...
<div class="max-w-6xl mx-auto py-10 px-4">
    <h1 class="text-3xl font-bold text-blue-800 mb-6">Detail Pindah Program Studi</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-blue-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Program Studi Lama</th>
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">Program Studi Baru</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">{{ $item->program_studi_asal }}</td>
                        <td class="px-4 py-3">{{ $item->program_studi_tujuan }}</td>
                        <td class="px-4 py-3">{{ $item->alasan }}</td>
                        <td class="px-4 py-3">{{ $item->created_at->format('d-m-Y') }}</td>

                        <!-- Status -->
                        <td class="px-4 py-3">
                            <form action="{{ route('admin.pindah-prodi.status', $item->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                                @method('PUT')
                                <select name="status"
