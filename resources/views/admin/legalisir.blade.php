...
<div class="max-w-6xl mx-auto py-10 px-4">
    <h1 class="text-3xl font-bold text-blue-800 mb-6">Data Pengajuan Legalisir</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-blue-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-gray-700 font-semibold">File Ijazah</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            @if ($item->file_ijazah)
                                <a href="{{ asset('storage/' . $item->file_ijazah) }}" target="_blank" class="text-blue-600 hover:underline">
                                    Lihat File
                                </a>
                            @else
                        ...
                        <!-- Status -->
                        <td class="px-4 py-3">
                            <form action="{{ route('admin.legalisir.status', $item->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                                @method('PUT')
                                <select name="status"
