...
<div class="max-w-xl mx-auto bg-white p-6 md:p-8 shadow-lg rounded-xl"> {{-- Shadow & rounding lebih jelas, padding responsif --}}
    <h2 class="text-2xl md:text-3xl font-semibold text-gray-800 mb-6 border-b pb-4"> {{-- Judul lebih besar, border bawah --}}
    ...
    {{-- Form --}}
    <form action="{{ route('seminar-proposal.submit') }}" method="POST" class="space-y-6"> {{-- Jarak antar field lebih besar --}}
        @csrf
        ...
        <div>
            ...
            <input type="text" name="judul" id="judul"
                   class="mt-1 block w-full px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out sm:text-sm"
                   required placeholder="Masukkan judul proposal tugas akhir">
        </div>
        ...
        <div class="pt-5"> {{-- Jarak di atas tombol --}}
            <button type="submit"
                ...
                Ajukan Seminar Proposal {{-- Teks tombol lebih deskriptif --}}
            </button>
        </div>
    </form>
</div>
